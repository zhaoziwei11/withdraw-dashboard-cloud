...
# 各站点失败核查所用的「提现状态」文本(页面状态下拉真实选项可能不同):
#   carrier(承运提现)/withdrawal(提现管理·车主提现) 选项含「提现失败」;
#   admin(提现拦截) 选项文本可能不同,
#   故以 status_text 为主、aliases 候选 + 模糊匹配兜底(见 set_withdraw_status)。
FAIL_STATUS_BY_SITE = {
    "carrier": "提现失败",
    "admin": "提现失败",
    "withdrawal": "提现失败",
}
FAIL_ALIASES_BY_SITE = {
    "carrier": None,
    "admin": ["提现失败", "失败", "已失败", "提现未成功"],
    "withdrawal": ["提现失败", "失败", "已失败", "提现未成功"],
}

# 各站点「待提现/需求二」统计时要剔除的状态文本。
# 采用"导出全部 + 本地按状态列过滤"的方式,避免页面状态筛选未生效导致失败记录混入待提现金额。
EXCLUDE_STATUSES_BY_SITE = {
    "carrier": ["提现失败"],
    "admin": ["提现失败", "失败"],
    "withdrawal": ["失败"],
}
# 失败原因列:承运提现页表头为「失败原因」,提现管理(车主提现)页表头为「失败」(2026-09-01 确认)
COL_FAIL_REASON = ["失败原因", "失败"]
COL_STATUS = "提现状态"
...
def compute_coeff_from_export(csv_path, label="近1月", exclude_threshold=0, exclude_statuses=None):
    ...
    exclude_statuses: 状态文本列表,命中则跳过(用于剔除"失败"/"提现失败"等,避免其混入待提现统计)。
    ...
    status_col = _find_col(fieldnames, ["提现状态", "状态", "处理状态"]) if exclude_statuses else None
    if not (date_col and amount_col):
        raise RuntimeError(f"表格列不匹配,需要包含:创建时间(或日期) + 金额。当前列: {fieldnames}")
    if exclude_statuses and not status_col:
        print(f"  [WARN] 表格未找到状态列,无法剔除 {exclude_statuses};当前列: {fieldnames}")
    ...
    for r in rows:
        # 按状态列剔除失败等不应计入待提现的记录
        if status_col and exclude_statuses:
    ...
    if exclude_statuses:
        print(f"  [OK] {label}: 有效行 {n} 条, 剔除状态 {exclude_statuses} 共 {excluded} 条, "
              f"0~{SPLIT_HOUR}点合计 ¥{total_partial:,.2f}, 0~24点合计 ¥{total_full:,.2f}")
...
def set_withdraw_status(page, status_text, aliases=None):
    """设置提现状态下拉框。

    不同站点(承运提现 / 提现管理·车主提现 / 提现拦截)页面状态下拉的真实选项文本可能不同:
    承运/车主提现为「提现失败」,提现拦截类页面可能叫「失败」等。匹配策略:
        1) 精确匹配 status_text; 2) 精确匹配 aliases 任一候选; 3) 模糊匹配(选项文本包含任一候选)。
    均失败则收集并 dump 该下拉真实选项,抛清晰错误(便于后续精确定制)。
    返回真实选中的文本,供后续校验/读取使用。
    """
    print(f"  -> 设置提现状态: {status_text}" + (f" (别名候选: {aliases})" if aliases else ""))
    res = page.evaluate(
        """(arg) => {
            const status_text = arg.status_text;
            const aliases = arg.aliases || [];
            function findSelectWrap() {
                let selectWrap = null;
            ...
            const selectWrap = findSelectWrap();
            if (!selectWrap) return {error: 'select not found'};

            const candidates = Array.from(new Set([status_text, ...aliases])).filter(Boolean);
            function pickFromVue() {
                const vue = selectWrap.__vue__ || selectWrap.__VUE__;
                if (!vue || !vue.options) return null;
                for (const cand of candidates) {
                    const opt = vue.options.find(o => (o.currentLabel || o.label || '') === cand);
                    if (opt) return {opt, via: 'vue'};
                }
                for (const cand of candidates) {
                    const opt = vue.options.find(o => ((o.currentLabel || o.label || '') + '').includes(cand));
                    if (opt) return {opt, via: 'vue-fuzzy'};
                }
                return null;
            }
            const pv = pickFromVue();
            if (pv && pv.opt && pv.opt.handleOptionSelect) {
                pv.opt.handleOptionSelect(pv.opt, true);
                return {ok: true, via: pv.via, selected: (pv.opt.currentLabel || pv.opt.label || '')};
            }
            ...
            function collectAndMatch() {
                ...
                for (const d of dropdowns) {
                    ...
                    for (const o of opts) {
                        ...
                        if (t) allOptions.push(t);
                    }
                    for (const o of opts) {
                        const t = o.textContent.trim();
                        if (candidates.includes(t)) { chosen = o; break; }
                    }
                    if (chosen) break;
                }
            }
            for (let i = 0; i < 30 && !chosen; i++) { collectAndMatch(); if (!chosen) { const s = Date.now(); while (Date.now() - s < 100) {} } }
            if (!chosen) {
                for (let i = 0; i < 30 && !chosen; i++) {
                    const dropdowns = document.querySelectorAll('.el-select-dropdown');
                    for (const d of dropdowns) {
                        const opts = d.querySelectorAll('.el-select-dropdown__item');
                        for (const o of opts) {
                            const t = o.textContent.trim();
                            if (candidates.some(c => t.includes(c))) { chosen = o; break; }
                        }
                        if (chosen) break;
                    }
                    if (chosen) break;
            ...
            if (!chosen) return {error: 'option not found', available: Array.from(new Set(allOptions))};
            chosen.click();
            return {ok: true, via: 'click', selected: chosen.textContent.trim()};
        }""",
        {"status_text": status_text, "aliases": aliases or []},
    )
    if not res.get("ok"):
        avail = res.get("available")
        msg = f"提现状态设置失败: {res}"
        if avail:
            msg += f"\n  [诊断] 该页面状态下拉真实可用选项: {avail}"
        raise RuntimeError(msg)
    page.wait_for_timeout(600)
    return res.get("selected") or status_text
...
def set_filters(page, start_date, end_date, status_text, aliases=None):
    """设置完整筛选条件并查询,查询后校验是否生效"""
    set_create_time_range(page, start_date, end_date)
    real_status = set_withdraw_status(page, status_text, aliases=aliases)
    click_query(page)
    verify_filters(page, real_status, start_date, end_date)
...
def run_fail_phase(debug=False, start_date=None, end_date=None):
    ...
    if page is None:
        return None
    try:
        set_filters(page, start, end, FAIL_STATUS_BY_SITE.get(SITE_NAME, "提现失败"), aliases=FAIL_ALIASES_BY_SITE.get(SITE_NAME))
...
def run_demand2_phase(debug=False):
    ...
    if page is None:
        return None
    try:
        # 导出当天(创建时间=今天)全部明细,本地按状态列剔除失败记录后,再按 创建时间 小时切 0~SPLIT_HOUR / 0~24
        csv_path = export_daily_csv(page, start, end, status="全部", key="today")
        try:
            exclude_statuses = EXCLUDE_STATUSES_BY_SITE.get(SITE_NAME, ["提现失败"])
